...
BEGIN
    ...
    BEGIN TRY
        ...
        -- Verificar si el proveedor tiene dependencias asociadas

        IF EXISTS (SELECT 1 FROM Detalle_Equipo WHERE id_proveedor = @ID_proveedor)

        BEGIN

            -- Lanzar un error si hay dependencias asociadas

            RAISERROR('No se puede eliminar el proveedor porque hay dependencias asociadas.', 16, 1);
...
-- PROCEDIMIENTO: [dbo].[ObtenerNombreIdDeZona]
...
-- PROCEDIMIENTO: [dbo].[P_ObtenerVlanNombreDeVlans]
...
-- PROCEDIMIENTO: [dbo].[P_ObtenerAreaIdDeAreas]
...
-- PROCEDIMIENTO: [dbo].[P_ObtenerMarcaIdDeMarcas]
...
-- PROCEDIMIENTO: [dbo].[P_ObtenerHistorialPingsParaIndexHistorial]
CREATE PROCEDURE [dbo].[P_ObtenerHistorialPingsParaIndexHistorial]
...
BEGIN
    ...
    WITH HistorialPings_CTE AS (
        ...
        SELECT
            ...
            ROW_NUMBER() OVER (ORDER BY HoraPing DESC) AS RowNum
    ...
    FROM HistorialPings_CTE

    WHERE RowNum BETWEEN (@PageNumber - 1) * @PageSize + 1 AND @PageNumber * @PageSize;



    -- Devuelve el total de registros como un segundo conjunto de resultados

    SELECT COUNT(*) AS TotalRecords FROM HistorialPings;
...
-- PROCEDIMIENTO: [dbo].[P_ObtenerUsuarioRol]
CREATE PROCEDURE P_ObtenerUsuarioRol
...
BEGIN
    ...
    WHERE u.Nombre_Usuario = @Nombre_Usuario AND u.Contrasena = @Contrasena
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_CrearUsuario]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_ObtenerUsuarios]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_ObtenerUsuarioPorID]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_ActualizarUsuario]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_EliminarUsuario]
CREATE PROCEDURE P_GRUD_EliminarUsuario
    @ID_usuario INT
AS
BEGIN
    IF (SELECT COUNT(*) FROM Usuarios) > 1
    BEGIN
        DELETE FROM Usuarios WHERE ID_usuario = @ID_usuario
    END
    ELSE
    BEGIN
        RAISERROR ('No se puede eliminar el último usuario.', 16, 1)
    END
END
GO
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_CrearDestinatario]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_ObtenerDestinatarios]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_ObtenerDestinatarioPorID]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_ActualizarDestinatario]
...
-- PROCEDIMIENTO: [dbo].[P_GRUD_EliminarDestinatario]
...
-- PROCEDIMIENTO: [dbo].[P_GetActivos_G]
CREATE PROCEDURE P_GetActivos_G
...
BEGIN
    ...
    SELECT
        ...
        SUM(CASE WHEN di.ping = 0 THEN 1 ELSE 0 END) AS Activo,

        SUM(CASE WHEN di.ping = 1 THEN 1 ELSE 0 END) AS Inactivo
...
BEGIN
    ...
    INSERT INTO DireccionesIp (IPV4, Estado, id_vlan, ping, Activa, UltimaHoraPing) 

    VALUES (@IPV4, @Estado, @id_vlan, 1, @Activa, GETDATE())
...
-- PROCEDIMIENTO: [dbo].[P_ObtenerHistorialPingsUltimos]
CREATE PROCEDURE [dbo].[P_ObtenerHistorialPingsUltimos]

    @ip NVARCHAR(50) -- Parámetro opcional para filtrar por IP específica

AS

BEGIN

    SELECT TOP 10 [ID_HistorialPing], [ip], [HoraPing], [ResultadoPing]

    FROM [HistorialPings]

    WHERE (@ip IS NULL OR [ip] = @ip)

    ORDER BY [HoraPing] DESC; -- Ordenado por la hora más reciente
...
CREATE PROCEDURE P_ActualizarPingDireccionIp
...
BEGIN
    ...
    SET 
        ping = @Ping,
        UltimaHoraPing = @UltimaHoraPing
    WHERE IPV4 = @DireccionIP;

    -- Verificación para asegurar que se actualizó un registro
    IF @@ROWCOUNT = 0
    BEGIN
        PRINT 'No se encontró una dirección IP con ese valor.';
    END
    ELSE
    BEGIN
...
CREATE PROCEDURE P_ObtenerCorreoSettings
AS
BEGIN
    -- Devolver siempre la primera fila
    SELECT TOP 1 SmtpServer, SmtpPort, EmailFrom, EmailPassword
    FROM CorreoSettings;
END;

GO

CREATE PROCEDURE P_ActualizarCorreoSettings
...
AS
BEGIN
    -- Validar si ya existe una fila
    IF EXISTS (SELECT 1 FROM CorreoSettings)
    BEGIN
        -- Actualizar la fila existente
        UPDATE CorreoSettings
        SET 
    ...
    BEGIN
        ...
        VALUES (@SmtpServer, @SmtpPort, @EmailFrom, @EmailPassword);
    END
END;
Go